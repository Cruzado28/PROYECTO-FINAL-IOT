...
void mostrarOLED() {
  ...
  if (millis() < msgHasta) {
    ...
    oled.setCursor(0, 50); oled.print(liveMode ? "Modo: FUNCIONA" : "Modo: PRUEBA");
  } else {
    oled.setCursor(0, 10); oled.print("SMART PARK");
    oled.setCursor(0, 25); oled.print(liveMode ? "FUNCIONAMIENTO" : "PRUEBA");
...
void wsEvent(WStype_t type, uint8_t* payload, size_t length) {
  if (type == WStype_CONNECTED) {
    ...
    enviarWS(String("{\"mode\":\"") + modoOperacion + "\",\"message\":\"ESP32 conectado\"}");
  }
    ...
    if (msg.indexOf("\"cmd\":\"mode\"") >= 0) {
      String requested = extraerCampo(msg, "mode");
      requested.toLowerCase();
      if (requested == "live" || requested == "funcionamiento") {
        liveMode = true;
        modoOperacion = "live";
        mostrarMensaje("MODO", "FUNCIONAMIENTO", 2000);
      } else {
        liveMode = false;
        modoOperacion = "test";
        mostrarMensaje("MODO", "PRUEBA", 2000);
      }
      enviarWS(String("{\"mode\":\"") + modoOperacion + "\"}");
    }
      ...
      if (liveMode) {
        enviarWS("{\"cmdStatus\":\"blocked\",\"message\":\"Control manual bloqueado en modo funcionamiento\"}");
        return;
      }
      int angle = ANGULO_REPOSO;
      int idx = msg.indexOf("\"angle\":");
      if (idx >= 0) angle = msg.substring(idx + 8).toInt();
      angle = constrain(angle, 0, 180);
      servoBarrera.write(angle);
      anguloServoActual = angle;
      mostrarMensaje("Servo manual", String(angle) + " grados", 1800);
      broadcastEstado();
    }
  }
}
...
void imprimirEstadoSerial() {
  Serial.println("------------------------------------");
  Serial.print("Modo            : "); Serial.println(liveMode ? "FUNCIONAMIENTO" : "PRUEBA");
