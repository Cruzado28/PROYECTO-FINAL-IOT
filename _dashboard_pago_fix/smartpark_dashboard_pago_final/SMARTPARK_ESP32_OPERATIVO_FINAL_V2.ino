...
// ============================================================
// SMART PARK - ESP32 FINAL V2 / DASHBOARD + PAGO RAPIDO
// ============================================================
// Cambios aplicados:
...
// - V2: lectura RFID más rápida, OLED sin bloqueo y dashboard sincronizado.
...
const unsigned long DEBOUNCE_TIME = 850;
uint8_t currentReader = 0;
bool rfidOk[NUM_LECTORES] = { false, false };
unsigned long ultimoOLED = 0;
const unsigned long OLED_REFRESH_MS = 180;
...
const unsigned long TARIFA_REFRESH_MS = 180000; // cada 3 min, para no poner lenta la lectura RFID
...
void reviewReader(uint8_t readerIndex) {
  for (uint8_t i = 0; i < NUM_LECTORES; i++) digitalWrite(SS_PINS[i], HIGH);
...
void revisarRFID() {
  for (uint8_t i = 0; i < NUM_LECTORES; i++) {
    bool ok = lectorResponde(i);
    rfidOk[i] = ok;
    if (!ok) {
      Serial.print("[RFID] Lector "); Serial.print(nombreLector(i)); Serial.println(" no responde por SPI. Reintentando init.");
      mfrc522[i].PCD_Init();
      delay(5);
      rfidOk[i] = lectorResponde(i);
    }
  }
}
...
// ============================================================
void broadcastEstado() {
  bool accesoOk = rfidOk[LECTOR_ACCESO];
  bool pagoOk = rfidOk[LECTOR_PAGO];
...
void imprimirEstadoSerial() {
  ...
  Serial.print("RFID ACCESO     : "); Serial.println(rfidOk[LECTOR_ACCESO] ? "OK" : "ERROR");
  Serial.print("RFID PAGO       : "); Serial.println(rfidOk[LECTOR_PAGO] ? "OK" : "ERROR");
...
void loop() {
  ...
  // La tarifa es editable desde la página. Se consulta cada 3 min para no frenar el RFID.
  actualizarTarifaDesdeWeb(false);

  if (millis() - ultimoRFIDCheck > 10000) {
  ...
  if (millis() - ultimoOLED > OLED_REFRESH_MS) {
    ultimoOLED = millis();
    mostrarOLED();
  }

  if (millis() - ultimoWS > 900) {
    ultimoWS = millis();
    broadcastEstado();
  }
