...
function registrarPagoLocalUnico(data = {}) {
  ...
  if (!ok || pago <= 0) return;
  try {
    const key = [data.fechaOperacion || state.operationDate, data.uid || "", data.espacio || data.cardData?.espacio || "", pago.toFixed(2), data.duracionMin || data.cardData?.duracionMin || "", data.fechaHoraOperacion || ""].join("|");
    const store = JSON.parse(localStorage.getItem(STORAGE_PAYMENTS) || "{}");
    const events = Array.isArray(store.events) ? store.events : [];
    if (events.some((event) => event.key === key)) return;
    const total = Number(store.total || 0) + pago;
    events.push({ key, pago, fecha: data.fechaOperacion || state.operationDate, hora: new Date().toISOString() });
    localStorage.setItem(STORAGE_PAYMENTS, JSON.stringify({ total, events: events.slice(-80) }));
  } catch {}
}
