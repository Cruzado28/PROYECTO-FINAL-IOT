...
function setMode(mode, notifyDevice = true) {
  ...
  if (subtitle) subtitle.textContent = live
    ...
    : "Modo prueba — permite probar RFID de acceso, pago por tiempo y barrera";

  if (banner) {
    banner.className = `mode-banner ${live ? "live" : "test"}`;
    banner.innerHTML = live
      ? '<i class="ti ti-player-play"></i><span>Modo funcionamiento activo: el RFID de acceso registra entrada/salida; el RFID de pago calcula el pago por tiempo; salida sin pago queda como incidencia.</span>'
      : '<i class="ti ti-info-circle"></i><span>Modo prueba activo: puedes probar lectores, pago por tiempo y abrir manualmente la barrera. El cálculo usa la tarifa editable de Tarifas y Beneficios.</span>';
  }
