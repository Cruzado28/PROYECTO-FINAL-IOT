...
function registrarIncidenciaLocal(data = {}, tipo = "Incidencia") {
  ...
  const key = [fh.fechaISO, fh.hora, uid, placa, tipo, data.message || ""].join("|");
  ...
  rows.push({
    ...
    tipo,
    placaUid: placa || uid || "—",
    detalle: data.message || "Incidencia registrada por el lector de acceso",
...
function handleAccessStatus(data) {
  ...
  else if (!ok && (tipo.includes("sin_pago") || tipo.includes("incid"))) {
    upsertHistorial(data, "Incidencia");
    registrarIncidenciaLocal(data, "Salida sin pago");
...
function handleIncidentStatus(data) {
  ...
  upsertHistorial(data, "Incidencia");
  registrarIncidenciaLocal(data, /doble/i.test(String(data.message || "")) ? "Doble ingreso" : "Incidencia");
  actualizarEspacioPorEvento(data, "Incidencia");
}
