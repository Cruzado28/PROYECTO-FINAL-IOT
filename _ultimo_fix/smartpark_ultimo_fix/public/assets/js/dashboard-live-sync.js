(function(){
  ...
  function readLive(){
    const live = readJson(STORAGE_LIVE_STATE, null);
    if (live?.fechaOperacion && live.fechaOperacion !== activeDate()) return null;
    return live;
  }
  ...
  function apply(payload){
    if (!payload || !payload.resumen) {
      setKpi("ESPACIOS TOTALES", "10", "Capacidad total");
      setKpi("ESPACIOS OCUPADOS", "0", "Sin registros en la fecha seleccionada");
      setKpi("ESPACIOS DISPONIBLES", "10", "Libres para esta fecha");
      setKpi("VEHÍCULOS ACTIVOS", "0", "Sin vehículos activos en esta fecha");
      setKpi("INGRESOS DEL DÍA", money(0), "Pagos acumulados por RFID");
      setKpi("TIEMPO PROMEDIO", duration(0), "Promedio en vivo");
      return;
    }
