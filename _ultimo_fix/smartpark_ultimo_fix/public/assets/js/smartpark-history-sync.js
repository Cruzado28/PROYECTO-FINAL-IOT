(function(){
  ...
  function currentLivePayload(){
    const live = readJson(STORAGE_LIVE_DASHBOARD, null);
    if (live?.fechaOperacion && live.fechaOperacion !== currentDate()) return null;
    return live;
  }
  ...
  function setDashboardDateInputs(){
    ["legacy-dashboard-date-input", "dashboard-date-input"].forEach(id => {
      const node = $(id);
      if (node && node.value !== currentDate()) node.value = currentDate();
    });
  }

  function changeOperationDate(date){
    const value = /^\d{4}-\d{2}-\d{2}$/.test(String(date||"")) ? String(date) : currentDate();
    localStorage.setItem(STORAGE_SIM_DATE, value);
    const from = $("h-from"), to = $("h-to");
    if (from) from.value = value;
    if (to) to.value = value;
    setDashboardDateInputs();
    renderHistoryFromStorage();
    syncLegacyDashboard();
    const frame = document.getElementById("dashboard-modern-frame");
    try { frame?.contentWindow?.postMessage({ type:"smartparking:set-date", date:value }, window.location.origin); } catch {}
  }

  function bindDashboardDateInputs(){
    ["legacy-dashboard-date-input", "dashboard-date-input"].forEach(id => {
      const node = $(id);
      if (!node || node.dataset.smartparkDateBound) return;
      node.dataset.smartparkDateBound = "1";
      node.value = currentDate();
      node.addEventListener("change", () => changeOperationDate(node.value));
    });
  }
