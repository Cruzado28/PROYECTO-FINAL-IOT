...
function normalizarEspacio(item, index = 0) {
  ...
  return {
    numero,
    estado: estadoLegible(item.estado || item.estadoSesion || item.status),
    placa,
    uid,
    pago: Number(item.pago || 0),
    duracionMin: Number(item.duracionMin || item.tiempoActualMinutos || 0),
    incidencia: Boolean(item.incidencia)
  };
}
...
function renderLiveSpaces() {
  ...
  grid.replaceChildren(...spaces.map((space) => {
    ...
    badge.textContent = estadoLegible(space.estado);
    top.append(num, badge);

    const plate = document.createElement("div");
    plate.className = "live-space-plate";
    plate.textContent = space.placa || "Libre";

    const meta = document.createElement("small");
    const uidCorto = space.uid ? `UID ${ultimos3UID(space.uid)}` : "Sin vehículo";
    const pago = space.pago ? ` · S/ ${Number(space.pago).toFixed(2)}` : "";
    meta.textContent = `${uidCorto}${pago}`;

    card.append(top, plate, meta);
    return card;
  }));
}
...
function handleAccessStatus(data) {
  ...
  const tag = tipo.includes("salida") ? "SALIDA" : tipo.includes("entrada") ? "ENTRADA" : "ACCESO";
  addLog(ok ? "ok" : "error", tag, data.message || (ok ? "Acceso autorizado." : "Acceso rechazado."));

  if (ok && tipo.includes("entrada")) actualizarEspacioPorEvento(data, "Estacionado");
  else if (ok && tipo.includes("salida")) actualizarEspacioPorEvento(data, "Libre");
  else if (!ok && (tipo.includes("sin_pago") || tipo.includes("incid"))) actualizarEspacioPorEvento(data, "Incidencia");
}
...
function handleIncidentStatus(data) {
  ...
  actualizarEspacioPorEvento(data, "Incidencia");
}
...
function handlePaymentStatus(data) {
  ...
  const pagoTexto = data.pago !== undefined ? ` Pago: S/ ${Number(data.pago || 0).toFixed(2)}` : "";
  const duracionTexto = data.duracionMin !== undefined ? ` · ${Number(data.duracionMin || 0)} min` : "";
  addLog(ok ? "ok" : "error", "PAGO", `${data.message || (ok ? "Pago aplicado." : "Pago rechazado.")}${pagoTexto}${duracionTexto}`);
  if (ok) actualizarEspacioPorEvento(data, "Pagado");
}
...
function setMode(mode, notifyDevice = true) {
  ...
  if (subtitle) subtitle.textContent = live
    ? "Funcionamiento — RFID de acceso valida entrada/salida y RFID de pago cobra"
    : "Modo prueba — también permite probar RFID, pagos y la barrera";

  if (banner) {
    banner.className = `mode-banner ${live ? "live" : "test"}`;
    banner.innerHTML = live
      ? '<i class="ti ti-player-play"></i><span>Modo funcionamiento activo: el acceso registra entrada/salida, el pago se realiza en el segundo RFID y las salidas sin pago quedan como incidencia.</span>'
      : '<i class="ti ti-info-circle"></i><span>Modo prueba activo: puedes probar lectores, pagos y abrir manualmente la barrera. El cálculo usa la tarifa editable de Tarifas y Beneficios.</span>';
  }
...
function toggleServo() {
  ...
  const current = Number(state.servoAcceso);
  const angle = current === 0 ? 90 : 0;
  if (wsSend({ cmd: "servo", id: "acceso", angle })) addLog("info", "CMD", `Servo acceso → ${angle}°`);
}
